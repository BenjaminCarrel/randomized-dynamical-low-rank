...
class ExponentialRungeKutta(MatrixOdeSolver):
    ...
    Applied only to ODEs of the form X' = A(X) + G(X)
    where A is linear (typically stiff) and G is non linear.
    """
    ...
    def __init__(self, 
        ...
        self.order = order
        self.name = 'Exponential Runge-Kutta, order ' + str(self.order)
    ...
    def solve_equivalent_ode(self, t_span: tuple, X0: Matrix, GX: Matrix):
        ...
        def equiv_ode(t: float, x: ndarray):
            ...
            return (GX + self.matrix_ode.linear_field(t, X)).flatten()
    ...
    def stepper_1(self, t_span: tuple, X0: Matrix):
        ...
        If the matrix ODE is of the form X' = AX + G(X), then this method is
        X1 = exp(h A) X0 + h phi1(h A) G(X0)
        which is equivalent to solve the ivp
        X' = A X + G(X_0), X(0) = X0
        on the time domain [0, h].
        """
        GX0 = self.matrix_ode.non_linear_field(t_span[0], X0)
        X1 = self.solve_equivalent_ode(t_span, X0, GX=GX0)
        return X1
    ...
    def stepper_2(self, t_span: tuple, X0: Matrix):
        ...
        """
        t_subspan = (t_span[0], t_span[0] + (t_span[1] - t_span[0]) / 2)
        GX0 = self.matrix_ode.non_linear_field(t_span[0], X0)
        X_half = self.solve_equivalent_ode(t_subspan, X0, GX=GX0)
        GX_half = self.matrix_ode.non_linear_field(t_subspan[1], X_half)
